 ...
 * <input type="text"> con un desplegable de sugerencias propio (ver
 * `attachAutocomplete` en autocomplete.js — este archivo debe cargarse
 * ANTES que catalogo.js en el template). Se escribe directo en vez de
 * revolver un desplegable larguísimo, y cargar algo que todavía no está
 * en el catálogo es simplemente escribirlo: no hace falta un paso aparte
 * de "+ nueva/o...".
 *
...
function initCatalogoSelects(catalogo, anioId, marcaId, modeloId, versionId, valoresIniciales) {
  ...
  // convertimos, solo lo dejamos listo para sumarle el mismo desplegable.
  ...
  function ordenar(lista) {
    return lista.slice().sort(function (a, b) { return String(a).localeCompare(String(b), "es"); });
  }
  ...
  function todosLosAnios() {
    ...
    return ordenar(out).reverse();
  }

  function marcasParaAnio(anio) {
    return ordenar(Object.keys(catalogo).filter(function (marca) { return marcaTieneAnio(marca, anio); }));
  }

  function modelosDe(marca, anio) {
    if (!catalogo[marca]) return [];
    return ordenar(Object.keys(catalogo[marca]).filter(function (modelo) { return modeloTieneAnio(marca, modelo, anio); }));
  }
  ...
  // --- Desplegables propios (ver autocomplete.js) ---
  if (anioEl) {
    attachAutocomplete(anioEl, function () { return todosLosAnios(); });
  }

  attachAutocomplete(marcaEl, function () {
    return anioEl ? marcasParaAnio(anioActual()) : Object.keys(catalogo).sort(function (a, b) { return a.localeCompare(b, "es"); });
  });

  if (modeloEl) {
    attachAutocomplete(modeloEl, function () {
      var marca = marcaEl.value.trim();
      var anio = anioActual();
      return marca ? modelosDe(marca, anio) : todosLosModelos(anio);
    });
    // Si todavía no eligieron marca y el modelo tipeado existe en una sola
    // marca del catálogo (para el año elegido, si hay), la completamos sola.
    modeloEl.addEventListener("input", function () {
      if (!marcaEl.value.trim()) {
        var marcas = marcasDeModelo(modeloEl.value.trim(), anioActual());
        if (marcas.length === 1) marcaEl.value = marcas[0];
      }
    });
  }

  if (versionEl) {
    attachAutocomplete(versionEl, function () {
      var marca = marcaEl.value.trim();
      var modelo = modeloEl ? modeloEl.value.trim() : "";
      return modelo ? versionesDe(marca, modelo, anioActual()) : [];
    });
  }

  // --- Pre-selección (editar un registro existente, o venir con datos ya sabidos) ---
  var vi = valoresIniciales || {};
  if (vi.anio && anioEl) anioEl.value = vi.anio;
  if (vi.marca) marcaEl.value = vi.marca;
