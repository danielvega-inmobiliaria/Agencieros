...
def _matches_por_pedido():
    """Corre el cruce completo para cada pedido activo ('buscando') y
    devuelve solo los que tienen al menos un match — reutilizado tanto por
    la pantalla de Matches como por el contador del menú, para no tener dos
    lugares con el mismo criterio."""
    pedidos = query("SELECT * FROM pedidos_clientes WHERE estado = 'buscando' ORDER BY created_at DESC")
    filas = []
    for p_raw in pedidos:
        ...
        if p.get("forma_pago") == "permuta" and p.get("permuta_marca"):
            matches_propios, matches_red = _buscar_matches_permuta(
                p["permuta_marca"], p.get("permuta_modelo"), excluir_pedido_id=p["id"]
            )
